...
def find_probs_unigram():
	...
	for sentences in corpus:
		...
		for words in sentences:	
			...
			total_count += 1
	with open(path+"unigram_freqs_1.txt", "wb") as myFile:
	...
	for word,val in probs_uni.iteritems():
		...
		probs_uni[word] = prob
	with open(path+"unigram_probs_1.txt", "wb") as myFile:
		pickle.dump(probs_uni, myFile)
			...
			if(sentences[i] not in freqs_bi.keys()):
				...
				if(i!=len(sentences)-1):
					if(sentences[i+1] in freqs_bi[sentences[i]].keys()):
						...
						freqs_bi[sentences[i]][sentences[i+1]] = 1
	with open(path+"bigram_freqs_1.txt", "wb") as myFile:
	...
	for sentences in corpus:
		...
		for i in range(2,len(sentences)):
			...
			if(pair in freqs_tri.keys()):
				...
				freqs_tri[pair] = {}
				freqs_tri[pair][sentences[i]] = 1
	with open(path+"trigram_freqs_1.txt", "wb") as myFile:
...
def get_probs_bi():
	...
	freqs_bi = {}
	freqs_uni = {}
	with open(path+"bigram_freqs_1.txt",'rb') as myFile:
		freqs_bi = pickle.load(myFile)
	with open(path+"unigram_freqs_1.txt",'rb') as myFile:
	...
	with open(path+"bigram_probs_1.txt", "wb") as myFile:
		pickle.dump(freqs_bi, myFile)
...
def get_probs_tri():
	...
	freqs_tri = {}
	freqs_bi = {}
	with open(path+"trigram_freqs_1.txt",'rb') as myFile:
	...
	with open(path+"trigram_probs_1.txt", "wb") as myFile:
		pickle.dump(freqs_tri, myFile)
...
# all results of these funtions are pickled :
...
probs_uni = {}
with open(path+"unigram_probs_1.txt", "rb") as myFile:
	probs_uni = pickle.load(myFile)
probs_bi = {}
with open(path+"bigram_probs_1.txt", "rb") as myFile:
	probs_bi = pickle.load(myFile)
probs_tri = {}
with open(path+"trigram_probs_1.txt", "rb") as myFile:
